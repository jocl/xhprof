 ...
 */
$base_path = rtrim(dirname($_SERVER['SCRIPT_URL']), '/\\');
...
function profiler_report ($url_params,
  ...
  $top_link_query_string = "$base_path/xhprof.html?" . http_build_query($base_url_params);
...
function full_report($url_params, $symbol_tab, $sort, $run1, $run2) {
  ...
  if ($diff_mode) {
    ...
    $base_url_params = xhprof_array_unset(xhprof_array_unset($url_params,
                                                             'run1'),
                                          'run2');
    $href1 = "$base_path/xhprof.html?" .
      http_build_query(xhprof_array_set($base_url_params,
                                        'run', $run1));
    $href2 = "$base_path/xhprof.html?" .
      http_build_query(xhprof_array_set($base_url_params,
                                        'run', $run2));
